...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.frame = CGRectMake(Main_Screen_Width, 0, Main_Screen_Width, Main_Screen_Height - 44 - 64 - 49);
    [self prepareUI];
...
-(void)load_Data:(BOOL)Refresh{
    ...
    [self.requestManager requestWebWithGETParaWith:@"Msg/unfriendShowHotMessage" Parameter:Parameterdic IsLogin:YES Finish:^(NSDictionary *resultDic) {
        [self hideHud];
        NSLog(@"成功");
        NSLog(@"返回==%@",resultDic);
