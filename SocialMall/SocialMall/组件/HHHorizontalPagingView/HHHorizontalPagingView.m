...
+ (HHHorizontalPagingView *)pagingViewWithHeaderView:(UIView *)headerView
    ...
    pagingView.horizontalCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, -44, Main_Screen_Width, Main_Screen_Height - 64)collectionViewLayout:layout];
    ...
    tempLayout.itemSize = CGSizeMake(pagingView.horizontalCollectionView.frame.size.width, pagingView.horizontalCollectionView.frame.size.height - 40);//pagingView.horizontalCollectionView.frame.size;
...
- (void)configureSegmentButtonLayout {
    if([self.segmentButtons count] > 0) {
        ...
        for(int i = 0; i < [self.segmentButtons count]; i++) {
            ...
            [segmentButton setImage:imgv.image forState:UIControlStateSelected];
